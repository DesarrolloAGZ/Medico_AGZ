...
namespace App\Http\Controllers\pages\pacientes;
...
class PacientesSeguimientoController extends Controller
{
  ...
  public function obtenerListadoPacientes(Request $request)
  {
    ...
    if ($request->filled('nombre_empleado')) {
      ...
      $paciente->where(function ($q) use ($nombre) {
    ...
    if ($request->filled('curp')) {
      $paciente->where('paciente.curp', 'iLIKE', "%$request->curp%");
    }
    # Filtro por numero de empleado
    if ($request->filled('numero_empleado')) {
      $paciente->where('paciente.gafete', 'iLIKE', "%$request->numero_empleado%");
    }

    $paciente = $paciente->groupBy([
      'paciente.id',
      'paciente.gafete',
      'paciente.nombre',
